...
def get_dot_graph(output, verbose=True):
    ...
    Args:
        output (tensorslow.Variable): Output variable from which the graph is
            constructed.
...
def reshape_sum_backward(gy, x_shape, axis, keepdims):
    """Reshape gradient appropriately for tensorslow.functions.sum's backward.

    Args:
        gy (tensorslow.Variable): Gradient variable from the output by backprop.
    ...
    Returns:
        tensorslow.Variable: Gradient variable which is reshaped appropriately
    """
    ndim = len(x_shape)
    tupled_axis = axis
    if axis is None:
...
def gradient_check(f, x, *args, rtol=1e-4, atol=1e-5, **kwargs):
    ...
    Args:
        ...
        x (`ndarray` or `tensorslow.Variable`): A traget `Variable` for computing
            the gradient.
...
def numerical_grad(f, x, *args, **kwargs):
    ...
    Args:
        f (callable): A function which gets `Variable`s and returns `Variable`s.
        x (`ndarray` or `tensorslow.Variable`): A target `Variable` for computing
            the gradient.
        *args: If `f` needs variables except `x`, you can specify with this
            argument.
        **kwargs: If `f` needs keyword variables, you can specify with this
...
def array_equal(a, b):
    ...
    Args:
        a, b (numpy.ndarray or cupy.ndarray or tensorslow.Variable): input arrays
            to compare
...
cache_dir = os.path.join(os.path.expanduser('~'), '.tensorslow')


def get_file(url, file_name=None):
    """Download a file from the `url` if it is not in the cache.

    The file at the `url` is downloaded to the `~/.tensorslow`.
